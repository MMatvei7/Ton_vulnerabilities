...
function resolveMapLiteral(exp, sctx, ctx) {
    const keyTy = (0, resolveDescriptors_1.getType)(ctx, exp.type.keyType);
    const valTy = (0, resolveDescriptors_1.getType)(ctx, exp.type.valueType);
    (0, resolveDescriptors_1.verifyMapType)(exp.type, valTy.kind === "struct");
    const keyFormalType = (0, resolveDescriptors_1.resolveTypeRef)(ctx, exp.type.keyType);
    const valueFormalType = (0, resolveDescriptors_1.resolveTypeRef)(ctx, exp.type.valueType);
    for (const { key, value } of exp.fields) {
        ...
        if (!(0, subtyping_1.isAssignable)(keyType, keyFormalType)) {
            (0, errors_1.throwCompilationError)(`Key of type "${(0, types_1.printTypeRef)(keyType)}" cannot be used as a key for map with keys of type "${(0, types_1.printTypeRef)(keyFormalType)}"`, key.loc);
        }
        if (!(0, subtyping_1.isAssignable)(valueType, valueFormalType)) {
            (0, errors_1.throwCompilationError)(`Value of type "${(0, types_1.printTypeRef)(valueType)}" cannot be used as a value for map with values of type "${(0, types_1.printTypeRef)(valueFormalType)}"`, key.loc);
        }
    }
    return registerExpType(ctx, exp, {
        kind: "map",
        key: keyTy.name,
        keyAs: exp.type.keyStorageType !== undefined
            ? (0, ast_helpers_1.idText)(exp.type.keyStorageType)
            : null,
        value: valTy.name,
        valueAs: exp.type.valueStorageType !== undefined
            ? (0, ast_helpers_1.idText)(exp.type.valueStorageType)
            : null,
    });
}
...
function resolveFieldAccess(exp, sctx, ctx) {
    ...
    if (src.kind === "ref_bounced" &&
        ...
        if (srcT.fields.length === 1) {
            (0, errors_1.throwCompilationError)(`Maximum size of the bounced message is 224 bits, but the ${(0, errors_1.idTextErr)(exp.field)} field of type ${(0, errors_1.idTextErr)(src.name)} cannot fit into it because its too big, so it cannot be accessed. Reduce the type of this field so that it fits into 224 bits`, exp.field.loc);
        }
        (0, errors_1.throwCompilationError)(`Maximum size of the bounced message is 224 bits, but the ${(0, errors_1.idTextErr)(exp.field)} field of type ${(0, errors_1.idTextErr)(src.name)} cannot fit into it due to the size of previous fields or its own size, so it cannot be accessed. Make the type of the fields before this one smaller, or reduce the type of this field so that it fits into 224 bits`, exp.field.loc);
    }
...
function resolveCall(exp, sctx, ctx) {
    ...
    if (src.kind === "ref") {
        ...
        if (f) {
            ...
            if (f.self) {
                const selfIsOptional = f.self.kind === "ref" ? f.self.optional : false;
                if (src.optional && !selfIsOptional) {
                    (0, errors_1.throwCompilationError)(`Cannot call method ${(0, errors_1.idTextErr)(exp.method)} on an expression of type "${src.name}?" (optional ${src.name}) without unwrapping it with "!!" first`, exp.loc);
                }
            }
            return registerExpType(ctx, exp, f.returns);
        }
...
function resolveExpression(exp, sctx, ctx) {
    switch (exp.kind) {
        ...
        case "map_value": {
            return (0, errors_1.throwInternalCompilerError)("Map value should never get here");
        }
        case "set_literal": {
