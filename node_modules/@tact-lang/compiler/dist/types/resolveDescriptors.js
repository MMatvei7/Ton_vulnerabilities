...
exports.verifyMapType = verifyMapType;
exports.resolveTypeRef = resolveTypeRef;
exports.computeGlobalVariablesUsages = computeGlobalVariablesUsages;
 ...
 */
function computeGlobalVariablesUsages(ctx) {
    const ast = (0, store_1.getRawAST)(ctx);
    const globalVariables = new Set();
    const handler = (node) => {
        (0, iterators_1.traverse)(node, (node) => {
            if (node.kind === "static_call") {
                const name = (0, ast_helpers_1.idText)(node.function);
                if (name === "inMsg") {
...
function resolveDescriptors(ctx, Ast) {
    ...
    for (const a of ast.types) {
        ...
        if (a.kind === "contract" && a.params === undefined) {
            ...
            if (init) {
                init.params.forEach((param) => (0, resolveABITypeRef_1.resolveABIType)(param));
            }
        }
    }
    for (const a of ast.types) {
    ...
    function copyTraits(contractOrTrait) {
        ...
        for (const funInContractOrTrait of contractOrTrait.functions.values()) {
            ...
            if (!overriddenFunction.isAbstract &&
                !overriddenFunction.isVirtual) {
                // override fun foo() { ... }
                // ^^^^^^^^
                const overrideLoc = funInContractOrTrait.ast.attributes.find((it) => it.type === "override")?.loc ?? funInContractOrTrait.ast.loc;
                (0, errors_1.throwCompilationError)(`Cannot override function "${funInContractOrTrait.name}" because function "${funInContractOrTrait.name}" does not have a virtual or abstract modifier in parent trait`, overrideLoc);
            }
        }
        ...
        for (const constantInContractOrTrait of contractOrTrait.constants.values()) {
            ...
            const overriddenConstant = contractOrTrait.traits
                .flatMap((t) => {
                const constant = t.constants.find((it) => it.name == constantInContractOrTrait.name);
                if (!constant)
                    return [];
                return [constant];
            })
            ...
            const iaAbstractOrVirtual = overriddenConstant.ast.attributes.find((a) => a.type === "virtual" || a.type === "abstract");
            if (!iaAbstractOrVirtual) {
                // override const A: Int = 10;
                // ^^^^^^^^
                const overrideLoc = constantInContractOrTrait.ast.attributes.find((it) => it.type === "override")?.loc ?? constantInContractOrTrait.ast.loc;
                (0, errors_1.throwCompilationError)(`Cannot override constant "${constantInContractOrTrait.name}" because constant "${constantInContractOrTrait.name}" does not have a virtual or abstract modifier in parent trait`, overrideLoc);
            }
        }
        ...
        for (const inheritedTrait of contractOrTrait.traits) {
            for (const traitFunction of inheritedTrait.functions.values()) {
                ...
                if (typeof previousInfo !== "undefined") {
                    ...
                    if (owner !== inheritedTrait &&
                        !traitFunction.isOverride &&
                        !method.isOverride) {
                        (0, errors_1.throwCompilationError)(`Both "${inheritedTrait.name}" and "${owner.name}" define method "${traitFunction.name}"`, contractOrTrait.ast.name.loc);
                    }
                }
