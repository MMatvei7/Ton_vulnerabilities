...
exports.evaluateRequireErrorString = evaluateRequireErrorString;
...
function resolveStringsInAST(ast, ctx, util) {
    (0, iterators_1.traverse)(ast, (node) => {
        if (node.kind === "static_call" && (0, ast_helpers_1.isRequire)(node.function)) {
            ...
            const resolved = (0, interpreter_1.ensureString)(evaluateRequireErrorString(node.args[1], ctx, util)).value;
...
function evaluateRequireErrorString(expr, ctx, util) {
    try {
        return (0, constEval_1.evalConstantExpression)(expr, ctx, util);
    }
    catch (e) {
        if (e instanceof errors_1.TactConstEvalError) {
            (0, errors_1.throwCompilationError)(`The second parameter of "require()" must be evaluated at compile time`, expr.loc);
        }
        throw e;
    }
}
