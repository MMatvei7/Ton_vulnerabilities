...
function writeAccessors(type, origin, ctx) {
    ...
    ctx.fun(ops_1.ops.typeFromTuple(type.name, ctx), () => {
        const returnType = type.fields.length === 0
            ? "tuple"
            : `(${type.fields.map((v) => (0, resolveFuncType_1.resolveFuncType)(v.type, ctx)).join(", ")})`;
        ctx.signature(`${returnType} ${ops_1.ops.typeFromTuple(type.name, ctx)}(tuple v)`);
        ctx.flag("inline");
        ctx.context("type:" + type.name);
        ctx.body(() => {
            // Resolve vars
