...
function ensureInt(val) {
    ...
    if (minTvmInt <= val.value && val.value <= maxTvmInt) {
        ...
        throwErrorConstEval(`integer '${(0, ast_printer_1.prettyPrint)(val)}' does not fit into TVM Int type`, val.loc);
    }
}
function ensureArgumentForEquality(val) {
    switch (val.kind) {
        ...
        case "map_value":
        case "struct_value": {
            throwErrorConstEval(`struct ${(0, ast_printer_1.prettyPrint)(val)} cannot be an argument to == operator`, val.loc);
            break;
        }
    }
}
function ensureRepeatInt(val) {
    ...
    if (minRepeatStatement <= val.value && val.value <= maxRepeatStatement) {
        ...
        throwErrorConstEval(`repeat argument '${(0, ast_printer_1.prettyPrint)(val)}' must be a number between -2^256 (inclusive) and 2^31 - 1 (inclusive)`, val.loc);
    }
}
...
const expectNumber = (node, loc) => {
    if (node.kind !== "number") {
        return throwErrorConstEval("Unexpected expression type", loc);
    }
    return node.value;
};
const expectAddress = (node, loc) => {
    if (node.kind !== "address") {
        return throwErrorConstEval("Unexpected expression type", loc);
    }
    return node.value;
};
const expectCell = (node, loc) => {
    if (node.kind !== "cell") {
        return throwErrorConstEval("Unexpected expression type", loc);
    }
    return node.value;
};
const expectBool = (node, loc) => {
...
class Interpreter {
    ...
    interpretMethodCall(ast) {
        switch ((0, ast_helpers_1.idText)(ast.method)) {
            case "asComment": {
                ...
                const comment = ensureString(this.interpretExpressionInternal(ast.self)).value;
        ...
        for (const field of structTy.fields) {
            ...
            if (fieldInit) {
                const defaultValue = field.default ??
                    (0, constEval_1.evalConstantExpression)(fieldInit, this.context, this.util);
    ...
    interpretMapValue(ast) {
        return ast;
    }
    interpretMapLiteral(ast) {
        const keyTy = (0, resolveDescriptors_1.getType)(this.context, ast.type.keyType);
        const valTy = (0, resolveDescriptors_1.getType)(this.context, ast.type.valueType);
        const res = serializers_1.mapSerializers.abiMatcher((0, resolveABITypeRef_1.getMapAbi)({
            kind: "map",
            key: keyTy.name,
            keyAs: ast.type.keyStorageType !== undefined
                ? (0, ast_helpers_1.idText)(ast.type.keyStorageType)
                : null,
            value: valTy.name,
            valueAs: ast.type.valueStorageType !== undefined
                ? (0, ast_helpers_1.idText)(ast.type.valueStorageType)
                : null,
        }, ast.loc));
        if (res === null) {
        ...
        const bocHex = keyExist((keyType, parseKey) => {
            return valueExist((valueType, parseValue) => {
                ...
                let dict = core_1.Dictionary.empty(keyType, valueType);
                for (const { key: keyExpr, value: valueExpr } of ast.fields) {
                    const keyValue = parseKey(this.interpretExpressionInternal(keyExpr), keyExpr.loc);
                    const valValue = parseValue(this.interpretExpressionInternal(valueExpr), valueExpr.loc);
    ...
    getKeyParser(src) {
        switch (src.kind) {
            case "int": {
                return (cb) => cb(core_1.Dictionary.Keys.BigInt(src.bits), expectNumber);
            }
            case "uint": {
                return (cb) => cb(core_1.Dictionary.Keys.BigUint(src.bits), expectNumber);
            }
            case "address": {
                return (cb) => cb(core_1.Dictionary.Keys.Address(), expectAddress);
            }
        }
    }
    getValueParser(src) {
        switch (src.kind) {
            case "int": {
                return (cb) => cb(core_1.Dictionary.Values.BigInt(src.bits), expectNumber);
            }
            case "uint": {
                return (cb) => cb(core_1.Dictionary.Values.BigUint(src.bits), expectNumber);
            }
            case "varint": {
                return (cb) => cb(core_1.Dictionary.Values.BigVarInt(src.length), expectNumber);
            }
            case "varuint": {
                return (cb) => cb(core_1.Dictionary.Values.BigVarUint(src.length), expectNumber);
            }
            case "address": {
                return (cb) => cb(core_1.Dictionary.Values.Address(), expectAddress);
            }
            case "cell": {
                return (cb) => cb(core_1.Dictionary.Values.Cell(), expectCell);
            }
            case "boolean": {
                return (cb) => cb(core_1.Dictionary.Values.Bool(), expectBool);
            }
            case "struct": {
                return (cb) => cb(core_1.Dictionary.Values.Cell(), (_, loc) => {
    ...
    interpretFieldAccess(ast) {
        ...
        if (valStruct.kind !== "struct_value") {
            throwErrorConstEval(`constant struct expected, but got ${(0, ast_printer_1.prettyPrint)(valStruct)}`, ast.aggregate.loc);
        }
    ...
    interpretStaticCall(ast) {
        switch ((0, ast_helpers_1.idText)(ast.function)) {
            case "ton": {
                ...
                const tons = ensureString(this.interpretExpressionInternal(ast.args[0]));
                if (tons.value.trim().length === 0) {
                    (0, errors_1.throwCompilationError)("ton() function requires a non-empty value", tons.loc);
                }
                const value = this.parseTonBuiltinValue(tons);
                if (typeof value === "undefined") {
                    (0, errors_1.throwCompilationError)("ton() function requires a valid number with no more than 10 digits after the decimal point", tons.loc);
                }
                if (value < 0) {
                    (0, errors_1.throwCompilationError)("ton() function requires a non-negative number", tons.loc);
                }
                try {
                    return ensureInt(this.util.makeNumberLiteral(value, ast.loc));
                }
                catch (e) {
            ...
            case "pow": {
                ...
                if (valExp.value < 0n) {
                    throwErrorConstEval(`${(0, errors_1.idTextErr)(ast.function)} builtin called with negative exponent ${(0, ast_printer_1.prettyPrint)(valExp)}`, ast.loc);
                }
                try {
            ...
            case "pow2": {
                ...
                if (valExponent.value < 0n) {
                    throwErrorConstEval(`${(0, errors_1.idTextErr)(ast.function)} builtin called with negative exponent ${(0, ast_printer_1.prettyPrint)(valExponent)}`, ast.loc);
                }
                try {
            ...
            case "cell":
                {
                    ensureFunArity(1, ast.args, ast.loc);
                    const str = ensureString(this.interpretExpressionInternal(ast.args[0]));
                    try {
                        return this.util.makeCellLiteral(core_1.Cell.fromBase64(str.value), ast.loc);
                    }
                    catch (_) {
                        throwErrorConstEval(`invalid base64 encoding for a cell: ${(0, ast_printer_1.prettyPrint)(str)}`, ast.loc);
                    }
                }
                break;
            case "slice":
            ...
            case "rawSlice":
                {
                    ...
                    if (!/^[0-9a-fA-F]*_?$/.test(str.value)) {
                        throwErrorConstEval(`invalid hex string: ${(0, ast_printer_1.prettyPrint)(str)}`, ast.loc);
                    }
            ...
            case "address":
                {
                    ...
                    try {
                        ...
                        if (address.workChain !== 0 &&
                            address.workChain !== -1) {
                            throwErrorConstEval(`${(0, ast_printer_1.prettyPrint)(str)} is invalid address`, ast.loc);
                        }
                        return this.util.makeAddressLiteral(address, ast.loc);
                    }
                    catch (_) {
                        throwErrorConstEval(`invalid address encoding: ${(0, ast_printer_1.prettyPrint)(str)}`, ast.loc);
                    }
                }
                break;
            case "newAddress": {
    ...
    parseTonBuiltinValue(tons) {
        try {
            const value = (0, core_1.toNano)(tons.value);
    ...
    interpretDestructStatement(ast) {
        ...
        if (val.kind !== "struct_value") {
            throwErrorConstEval(`destructuring assignment expected a struct, but got ${(0, ast_printer_1.prettyPrint)(val)}`, ast.expression.loc);
        }
